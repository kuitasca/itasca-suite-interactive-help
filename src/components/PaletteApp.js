...
const PaletteApp = () => {
  ...
  // Search index
  const handleSearchIndex = useCallback((query, aiResultData = null) => {
    const tokens = query ? (query.trim() ? query.toLowerCase().split(/\s+/) : []) : [];
  ...
  const handleAiSend = async () => {
    ...
    try {
      ...
      if (data.type === 'commands') {
        setExpandedRows({});
        handleSearchIndex(null, data.results);
      } else {
